...
static void *const YFTabBarContext = (void*)&YFTabBarContext;

@interface YFTabBar ()

/** 发布按钮 */
@property (nonatomic, strong) UIButton<YFPlusButtonSubclassing> *plusButton;
...
- (instancetype)sharedInit {
    if (YFExternPlusButton) {
        self.plusButton = YFExternPlusButton;
        [self addSubview:(UIButton *)self.plusButton];
    }
    // KVO注册监听
    _tabBarItemWidth = YFTabBarItemWidth;
    [self addObserver:self forKeyPath:@"tabBarItemWidth" options:NSKeyValueObservingOptionNew context:YFTabBarContext];
    return self;
}

- (CGSize)sizeThatFits:(CGSize)size {
    CGSize sizeThatFits = [super sizeThatFits:size];
    CGFloat height = [self yf_tabBarController].tabBarHeight;
    if (height > 0 && !RT_IS_IPHONE_X && !RT_IS_IPHONE_XS && RT_IS_IOS_11) {
        sizeThatFits.height = [self yf_tabBarController].tabBarHeight;
    }
    return sizeThatFits;
}
...
- (void)layoutSubviews {
    ...
    YFTabBarItemWidth = taBarWidth / YFTabbarItemsCount;
    self.tabBarItemWidth = YFTabBarItemWidth;
...
//    NSUInteger plusButtonIndex = [self plusButtonIndex];
    ...
    //            childViewX = buttonIndex * YFTabBarItemWidth + YFPlusButtonWidth;
    //        } else {
    //            childViewX = buttonIndex * YFTabBarItemWidth;
    //        }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if(context != YFTabBarContext) {
        [super observeValueForKeyPath:keyPath ofObject:object change:change context:context];
        return;
    }
    if(context == YFTabBarContext) {
        [[NSNotificationCenter defaultCenter] postNotificationName:YFTabBarItemWidthDidChangeNotification object:self];
    }
}
...
- (NSUInteger)plusButtonIndex {
    ...
    if ([[self.plusButton class] respondsToSelector:@selector(indexOfPlusButtonInTabBar)]) {
        ...
        plusButtonIndex = YFTabbarItemsCount * 0.5;
    }
    YFPlusButtonIndex = plusButtonIndex;
...
- (NSArray *)tabBarButtonFromTabBarSubviews:(NSArray *)tabBarSubviews {
    ...
    [tabBarSubviews enumerateObjectsUsingBlock:^(UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj yf_isTabButton]) {
            [tabBarButtonMutableArray addObject:obj];
        }
    }];
    if (YFPlusChildViewController) {
        [tabBarButtonMutableArray removeObjectAtIndex:YFPlusButtonIndex];
    }
...
- (void)setupTabImageViewDefaultOffset:(UIView *)tabBarButton {
    ...
    if (shouldCustomizeImageView && !RT_IS_IPHONE_X && !RT_IS_IPHONE_XS) {
